  ...
  const metadata = {
    ...
    participants: getBinaryNodeChildren(community, "participant").map(
      ({ attrs }) => {
        ...
        const phone = attrs.phone_number ? jidNormalizedUser(attrs.phone_number) : (isPnUser(id) ? id : undefined);
        const lid = attrs.lid ? jidNormalizedUser(attrs.lid) : (isLidUser(id) ? id : undefined);
        return {
          id: id,
          pn: phone,
          lid: lid,
