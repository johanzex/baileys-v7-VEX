 ...
 */ export const makeSocket = (config) => {
  ...
  const syncDisabled =
    PROCESSABLE_HISTORY_TYPES.map((syncType) =>
      config.shouldSyncHistoryMessage({ syncType: syncType }),
  ...
  /** ephemeral key pair used to encrypt/decrypt communication. Unique for each connection */ const ephemeralKeyPair =
    Curve.generateKeyPair();
  /** WA noise protocol wrapper */ const noise = makeNoiseHandler({
  ...
  /** send a raw buffer */ const sendRawMessage = async (data) => {
    if (!ws.isOpen) {
  ...
  /** send a binary node */ const sendNode = (frame) => {
  ...
  /** send a query, and wait for its response. auto-generates message ID if not provided */ const query =
    async (node, timeoutMs) => {
      if (!node.attrs.id) {
  ...
  const fetchDevices = async (jids) => {
    ...
    const results = await executeUSyncQuery(usyncQuery);
    if (results) {
      const extracted = results.list.flatMap((item) => {
        const { id, devices } = item;
        const deviceList = devices?.deviceList || [];
        return deviceList.map((d) =>
          jidEncode(jidDecode(id).user, d.isHosted ? "hosted" : "s.whatsapp.net", d.id),
        );
      });
      return extracted;
    }
  ...
  let qrTimer;
  let closed = false;
  /** log & process any unexpected errors */ const onUnexpectedError = (
    err,
    msg,
  ) => {
    logger.error({ err: err }, `unexpected error in '${msg}'`);
  };
  /** await the next incoming message */ const awaitNextMessage = async (
    sendMsg,
  ) => {
    if (!ws.isOpen) {
      throw new Boom("Connection Closed", {
  ...
  /** connection handshake */ const validateConnection = async () => {
  ...
  /** generates and uploads a set of pre-keys to the server */ const uploadPreKeys =
    async (count = MIN_PREKEY_COUNT, retryCount = 0) => {
  ...
  const onMessageReceived = async (data) => {
    await noise.decodeFrame(data, (frame) => {
      ...
      if (!(frame instanceof Uint8Array)) {
        ...
        /* Check if this is a response to a message we sent */ anyTriggered =
          ws.emit(`${DEF_TAG_PREFIX}${msgId}`, frame) || anyTriggered;
        /* Check if this is a response to a message we are expecting */ const l0 =
          frame.tag;
  ...
  /** i have no idea why this exists. pls enlighten me */ const sendPassiveIq =
    (tag) =>
      query({
        tag: "iq",
  ...
  /** logout & invalidate connection */ const logout = async (msg) => {
  ...
  ws.on("CB:iq,type:set,pair-device", async (stanza) => {
    ...
    let qrMs = qrTimeout || 6e4; // time to let a QR live
    const genPairQR = () => {
      if (!ws.isOpen) {
        return;
      }
      ...
      qrMs = qrTimeout || 2e4; // shorter subsequent qrs
    };
    genPairQR();
  });
  ...
  const sendUnifiedSession = async () => {
    ...
    const node = {
      ...
      attrs: {},
      content: [
        { tag: "unified_session", attrs: { id: getUnifiedSessionId() } },
      ],
    };
    try {
      await sendNode(node);
  ...
  return {
    ...
    updateServerTimeOffset: updateServerTimeOffset,
    sendUnifiedSession: sendUnifiedSession,
